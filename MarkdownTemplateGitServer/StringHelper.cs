...
namespace MarkdownTemplateGitServer
{
    ...
    {
        readonly int[,] _tempMatrix;
        private readonly string _firstString;
        private readonly string _secondString;

        public StringHelper(string firstString, string secondString)
        {
            _tempMatrix = new int[firstString.Length + 1, secondString.Length + 1];

            _firstString = firstString;
            _secondString = secondString;
        }

        public static List<string> LcsBack(List<string> a, List<string> b)
        {
            var test = a.Concat(b).Distinct().ToArray();
            var list1 = a.Select(t => Array.IndexOf(test, t));
            var list2 = b.Select(t => Array.IndexOf(test, t));

            var tempResult = LcsBackInternal(list1.ToList(), list2.ToList());
            return null;
        }



        public static List<int> LcsBackInternal(List<int> a, List<int> b)
        {
            if (a == null || b == null || !a.Any() || !b.Any())
                return new List<int>();
            var aSub = a.Take(a.Count - 1).ToList();
            var bSub = b.Take(b.Count - 1).ToList();

            if (a[a.Count - 1] == b[b.Count - 1])
            {
                var list = LcsBackInternal(aSub, bSub);
                list.Add(a[a.Count - 1]);
                return list;
            }
            List<int> x = LcsBackInternal(a, bSub);
            List<int> y = LcsBackInternal(aSub, b);
